...
for(let t=start;t<now;t+=STEP){const d=new Date(t),h=d.getUTCHours()+d.getUTCMinutes()/60;let bg=6.5;if(h>=2&&h<4)bg-=1.2;bg+=(Math.random()-0.5)*0.5;const r={_id:'e'+t,date:t,dateString:d.toISOString(),sgv:Math.round(Math.max(2.5,Math.min(18,bg))*MG),type:'sgv',direction:'Flat'};entries.push(r);idx.set(t,entries.length-1);}
...
for(const scheme of ['light','dark']){
  const page=await browser.newPage({viewport:{width:390,height:844},isMobile:true,hasTouch:true,deviceScaleFactor:2,colorScheme:scheme});
  ...
  await page.screenshot({path:`/tmp/diametric-tests/nav-${scheme}.png`});
  console.log(scheme,'errors:',errors.length?errors:'geen');
  await page.close();
}
await browser.close();
