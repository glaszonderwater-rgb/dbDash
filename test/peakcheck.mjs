...
const REAL_ISF=2.6,mdl=model(PEAK,DIA);
...
for(let day=start;day<now;day+=DAY){const ct=day+15*3600e3;treatments.push({_id:'k'+ct,created_at:new Date(ct).toISOString(),eventType:'Correction Bolus',type:'NORMAL',insulin:1.5});
...
await page.waitForFunction(()=>!document.getElementById('secProfile').hidden,{timeout:40000}).catch(()=>{});
await page.waitForTimeout(300);
const obs=await page.evaluate(()=>document.querySelector('#profile .obs')?.textContent||'');
const m=obs.match(/steilste daling na je correcties ligt mediaan rond ([\d]+) min/i);
const measured=m?+m[1]:null;
console.log('gemeten piektijd:', measured, 'min (model 75)');
console.log('binnen band 55–95:', measured!=null && measured>=55 && measured<=95 ? 'JA' : 'NEE');
console.log('sluit aan (|Δ|≤20):', /sluit aan bij je ingestelde piektijd/.test(obs) ? 'JA' : 'NEE');
