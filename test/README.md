...
- `askaicheck` — optionele AI-laag (eigen sleutel/endpoint): de "🤖 Vraag de AI"-knop verschijnt alleen als ingeschakeld, stuurt naar een gemockte OpenAI-compatibele endpoint alléén de samenvattende feiten (géén ruwe `sgv`/sensorstroom) met een harde "no dosing"-guardrail in de system-prompt, toont het antwoord met AI-badge + "wat wordt verstuurd", en handelt fouten netjes af
