...
for(let day=start;day<now;day+=DAY){ corr(day,8,1.5,'ko'); corr(day,15,2.6,'km'); }
...
const r=await page.evaluate(()=>({
  ...
  cards:[...document.querySelectorAll('#proposals .pcard')].map(c=>({
    ...
    rows:[...c.querySelectorAll('.pdrow')].map(rw=>({
      label:rw.querySelector('.pdlabel')?.childNodes[0]?.textContent.trim(),
      meta:rw.querySelector('.pdmeta')?.textContent,
      cands:[...rw.querySelectorAll('.pcand')].map(x=>({
        tag:x.querySelector('.pctag')?.textContent.replace(/\s+/g,' ').trim(),
        val:x.querySelector('.pcval')?.textContent.replace(/\s+/g,' ').trim(),
        dir:x.querySelector('.pcdir')?.textContent,
