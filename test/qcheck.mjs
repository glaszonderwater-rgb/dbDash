...
for(let day=start;day<now;day+=DAY){for(const [hh,carbs,ins] of [[8,40,4],[13,60,6],[19,70,7]]){const mt=day+hh*3600e3;treatments.push({_id:'t'+mt,created_at:new Date(mt).toISOString(),eventType:'Meal Bolus',carbs,insulin:ins});for(let k=1;k<=3;k++){const st=mt+k*12*60e3;treatments.push({_id:'t'+st,created_at:new Date(st).toISOString(),eventType:'SMB',insulin:0.4,isSMB:true});}for(let tt=mt;tt<=mt+4*3600e3;tt+=STEP){const i=idx.get(tt);if(i==null)continue;const min=(tt-mt)/60000;entries[i].bg+=(ISF/IC)*carbs*(1-Math.exp(-min/50))-ISF*ins*(1-mdl.iob(min));}}
  if(Math.floor((day-start)/DAY)%3===0){const c1=day+22*3600e3;treatments.push({_id:'c'+c1,created_at:new Date(c1).toISOString(),eventType:'Correction Bolus',carbs:0,insulin:2});treatments.push({_id:'c'+(c1+40*60e3),created_at:new Date(c1+40*60e3).toISOString(),eventType:'Correction Bolus',carbs:0,insulin:2});for(let tt=day+24*3600e3;tt<day+24*3600e3+70*60e3;tt+=STEP){const i=idx.get(tt);if(i!=null)entries[i].bg=3.3+Math.random()*0.3;}}
  // beweeg-events om exerciseEffect te raken
  const xt=day+16*3600e3;treatments.push({_id:'x'+xt,created_at:new Date(xt).toISOString(),eventType:'Exercise',duration:45,notes:'hardlopen'});
  for(let tt=xt;tt<=xt+90*60e3;tt+=STEP){const i=idx.get(tt);if(i!=null)entries[i].bg=Math.max(3.6,entries[i].bg-2.5);}
}
...
const r=await page.evaluate(()=>({vis:!document.getElementById('secQuestions').hidden,qa:[...document.querySelectorAll('#questions .qa')].map(x=>({q:x.querySelector('.q').textContent,a:x.querySelector('.a').textContent}))}));
console.log('secQuestions zichtbaar:',r.vis,'| aantal:',r.qa.length);
for(const {q,a} of r.qa) console.log('\nQ:',q,'\nA:',a);
