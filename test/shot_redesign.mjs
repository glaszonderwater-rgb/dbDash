...
for(const scheme of ['light','dark']){
  const page=await browser.newPage({viewport:{width:390,height:1000},isMobile:true,hasTouch:true,deviceScaleFactor:2,colorScheme:scheme});
  ...
  await page.screenshot({path:OUT+`rd-${scheme}-top.png`});
  if(scheme==='light') await page.screenshot({path:OUT+`rd-full.png`,fullPage:true});
